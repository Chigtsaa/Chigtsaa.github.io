class OfferModal extends HTMLElement {
  ...
  async handleConfirm() {
    ...
    const added = this.addToDeliveryCart(this.currentData);
    if (!added) {
      return;
    }
  ...
  addToDeliveryCart(data) {
    ...
    const key = `${title}|${meta}|${price}`;

    const existing = items.find((item) => item.id === key);
    if (existing) {
      alert("Энэ хүргэлт аль хэдийн сагсанд байна.");
      return false;
    }
    if (items.length >= 4) {
      alert("Хамгийн ихдээ 4 хүргэлт сонгоно.");
      return false;
    }
    items.push({
      id: key,
